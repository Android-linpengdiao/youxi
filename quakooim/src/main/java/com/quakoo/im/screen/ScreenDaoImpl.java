...
public class ScreenDaoImpl implements UserDao {

    UserDBHelper dbHelper;

    public ScreenDaoImpl(Context context) {
    ...
    public synchronized void insertThread(User screenInfo) {
        ...
        db.insert("screen_user", null, values);
        db.close();
    }
    ...
    public synchronized void deleteThread(String master, String user_id) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        int count = db.delete("screen_user", "master = ? and user_id = ?", new String[]{master, user_id});
        db.close();
    }
    ...
    public synchronized void updateThread(String master, String user_id) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        ContentValues values = new ContentValues();
        db.update("screen_user", values, "master = ? and user_id = ? ", new String[]{master, user_id});
    }
    ...
    public synchronized List<User> queryThread(String master, String user_id) {
        ...
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.query("screen_user", null, "master = ?  and user_id = ?",
    ...
    public synchronized List<User> queryThreadAll(String master) {
        List<User> list = new ArrayList<>();
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.query("screen_user", null, "master = ?",
